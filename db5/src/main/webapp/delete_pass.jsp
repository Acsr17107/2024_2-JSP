    ...
    <div class="container">
        ...
        <%
            ...
            if (isPasswordCorrect) {
                ...
                try {
                    conn = DriverManager.getConnection(dbURL, dbUser, dbPassword);
                    String deleteSQL = "DELETE FROM tblboard WHERE num = ?";
                    pstmt = conn.prepareStatement(deleteSQL);
                    pstmt.setInt(1, Integer.parseInt(num));
                    int rowsAffected = pstmt.executeUpdate();
                    if (rowsAffected > 0) {
                        // 삭제 성공
                        response.sendRedirect("listboard.jsp?message=게시물이 삭제되었습니다.");
                    } else {
                        out.println("<p class=\"error\">삭제에 실패했습니다. 다시 시도해주세요.</p>");
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
        ...
        <!-- 비밀번호 입력 폼 -->
        <form action="delete_pass.jsp" method="post">
